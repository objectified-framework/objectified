...
(async () => {
  ...
  for(const dto of Object.keys(schemas)) {
    ...
    dtoBody += dtoHeader;
    dtoBody += 'import { ApiProperty, ApiPropertyOptional } from \'@nestjs/swagger\';\n\n';
    ...
    for(const property of Object.keys(dtoProperties)) {
      ...
      let requiredFlag = dtoRequired.includes(property) ? '' : '?';
      let optionalFlag = dtoRequired.includes(property) ? '' : 'Optional';

      dtoBody += `  @ApiProperty${optionalFlag}(${propertiesDump.substring(0, propertiesDump.length - 1)}})\n`;
      dtoBody += `  ${property}${requiredFlag}: ${propertyToType(dtoProperties[property])};\n\n`;
    }
  ...
  const indexFilename = `${DTO_DIRECTORY}/index.ts`;
