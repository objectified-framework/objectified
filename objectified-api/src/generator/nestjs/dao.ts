...
export function generateDaos(daoDirectory: string, openapi: any) {
  ...
  for (const dao of Object.keys(schemas)) {
    ...
    const description =
      schemas[dao].description.trim().replaceAll("\n", "\n * ") ?? "";
...
`;
  ...
  fs.copyFileSync(
    path.join(__dirname, "../assets/BaseDao.ts"),
    `${daoDirectory}/BaseDao.ts`,
  );
  console.log(`  > Copied BaseDao.ts to ${daoDirectory}/BaseDao.ts`);

  fs.copyFileSync(
    path.join(__dirname, "../assets/DaoUtils.ts"),
    `${daoDirectory}/DaoUtils.ts`,
  );
