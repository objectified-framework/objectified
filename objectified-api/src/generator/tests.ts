...
const TESTS_DIRECTORY: string = 'src/tests/generated';
...
(async () => {
  ...
  program
    .option('-o, --out <directory>', 'output directory for tests', TESTS_DIRECTORY)
    .addOption(new Option('-g <generator>', 'output generator to use').choices(generatorDirectories))
    .parse();

  console.log(`Tests Generator: ${VERSION}`);

  fs.rmSync(TESTS_DIRECTORY, { recursive: true, force: true });
  fs.mkdirSync(TESTS_DIRECTORY, { recursive: true });

  console.log(`Generating Tests`);

  const controllerGenerator = require(`./${program.opts().g}`);

  controllerGenerator.generateTests(program.opts().out, openapi);
})();
