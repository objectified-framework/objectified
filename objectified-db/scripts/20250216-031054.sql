...
BEGIN
    ...
    SELECT jsonb_build_object(
        ...
        'properties', COALESCE(jsonb_object_agg(
            -- Ensure no null property names
            COALESCE(cp.name, p.name, 'unknown_property'),
            jsonb_strip_nulls(
                jsonb_build_object(
                    'type', LOWER(dt.name),
                    ...
                    '$ref',
                    CASE
                        WHEN p.class_id IS NOT NULL
                        THEN format('#/components/schemas/%s', cref2.name)
                        ELSE NULL
                    END
                )
            )
        ), '{}'::jsonb
    ),  -- Ensure an empty object if no properties exist
    'required',
    CASE
        WHEN EXISTS (
